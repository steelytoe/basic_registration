...
app.controller('registrationController', ['$scope', '$http', 'global', '$timeout', '$location', '$anchorScroll', '$uibModal', '$filter','$location','$routeParams', function($scope, $http, global, $timeout, $location, $anchorScroll, $uibModal, $filter, $location, $routeParams) {
	...
	function eventHeaderSelected(){
		...
		switch(defaultParticipant.evnhId) {
			...
			case 13:
				categoryField.options = [
					{name:"Labuan Bajo - Maumere (Category Closed)",value:33,disabled:true},
	...
	$scope.datepickers = {
		...
		changed : function(data, field, f){
			...
			data.selected.date = $filter('date')(data.selected.date, 'yyyy-MM-dd');
			console.log(data);
		}
	};
	...
	if (defaultParticipant.evnhId != 7&&defaultParticipant.evnhId != 9){	
		$scope.fields.push({
			type : 'option',
			name : 'evpaJersey',
	...
	$scope.sendData = function(){
		...
		$scope.sending = true;
		$http.post(global.API.registration, $scope.form).then(function(success){
			$scope.sending = false;
		...
		if ($scope.form&&$scope.form.payType){
			...
			switch($scope.form.payType){
				...
				case '2':
					...
					break;
				case '3':
					$scope.data.transaction.fee = 7500;
					$scope.data.transaction.total_amount = amount+7500;
					break;
			}
		}
	});	
